...
def main():
    ...
    # Compare & collect matches
    matched_paths = set()     # set of abs paths that matched at least one file
    index_by_abs = {}         # abs -> (root_idx, label, root, abs, rel) for copying
    for e in all_files:
        index_by_abs[e[3]] = e
    ...
    for A, B in pairs:
        ...
        _, _, _, path2, _ = B
        try:
            d1 = load_first_yaml_doc(path1) or {}
            d2 = load_first_yaml_doc(path2) or {}
        except Exception:
        ...
        if is_match:
            matched_pairs += 1
            matched_paths.add(path1)
            matched_paths.add(path2)
